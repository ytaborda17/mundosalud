...
class Menu extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('label, padre, url, active', 'required'),
			array('padre, estatus, sort', 'numerical', 'integerOnly'=>true),
			array('label', 'length', 'max'=>100),
			array('operation', 'length', 'max'=>50),
			array('url', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, label, padre, estatus, operation, url, active, sort', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'metadatas' => array(self::HAS_MANY, 'Metadata', 'menu_id'),
	...
	public function getListed($activeItem="") {
   	...
   	$returnarray = array(
   		...
   		'url' => $this->url!="" ? array('/'.$this->url) : array(),
		);
